...
*                                   l_ok.
     WHEN 'PO'.
       ...
       PERFORM frm_check_fields USING l_subrc.
       IF l_subrc NE 0.
       ...
       SELECT SINGLE qmnum
        ...
        WHERE qmnum = @zcs_po_head-qmnum
          AND status = '1'.
       IF sy-subrc EQ 0.
         MESSAGE s000(zsgcn) WITH  l_qm 'PO' 'has already generated!' DISPLAY LIKE 'E'.
         EXIT.
       ENDIF.
     ...
     WHEN 'SAVE'.
       ...
       SELECT SINGLE qmnum
            ...
            WHERE qmnum = @zcs_po_head-qmnum
              AND status = '1'.
       IF sy-subrc EQ 0.
         MESSAGE s000(zsgcn) WITH 'PO:'l_qm1  'have already generated!' DISPLAY LIKE 'E'.
         EXIT.
       ENDIF.
...
*         UPDATE zcs_po_head
*           SET lifnr = zcs_po_head-lifnr
*               waers = zcs_po_head-waers
*               ekorg = zcs_po_head-ekorg
*               ekgrp = zcs_po_head-ekgrp
*          WHERE qmnum = zcs_po_head-qmnum.
       ENDIF.
       ...
       LOOP AT g_ztc_itab INTO ls_line WHERE asnum IS NOT INITIAL.
         ...
         lt_item-zzj =  lt_item-tbtwr *  lt_item-menge.
...
*----------------------------------------------------------------------*
 FORM frm_create_po .
   ...
   IF g_ztc_itab[] IS INITIAL.
     SELECT *
       FROM zcs_po_item
       INTO CORRESPONDING FIELDS OF TABLE g_ztc_itab
       WHERE qmnum = zcs_po_head-qmnum.
   ENDIF.
...
*po header fill
   ...
   ls_header-ref_1 = zcs_po_head-qmnum.
...
*     ls_poitem-material = ls_line-asnum.
       ...
       ls_poitem-tax_code = zcs_po_head-mwskz.
...
*** ACCOUNT
       ...
       ELSEIF l_ktokk = 'SG02'.
         ls_poaccount-gl_account = '0070760000'.
       ENDIF.
...
*        WHERE kdauf = @l_vbeln.
       SELECT SINGLE aufnr
         FROM vbep
         WHERE vbeln = @l_vbeln
         AND ( posnr = '10' OR posnr = '11' )
         AND aufnr <> @space
         INTO @DATA(l_aufnr).

       ls_poaccount-orderid = l_aufnr.
       ls_poaccount-co_area = '4800'.
       ls_poaccount-tax_code = zcs_po_head-mwskz.
...
*  <--  p2        text
*----------------------------------------------------------------------*
 FORM frm_check_fields USING p_subrc TYPE sy-subrc.

   IF zcs_po_head-qmnum IS INITIAL.
     MESSAGE s000 WITH '字段:服务通知单 必输!'  DISPLAY LIKE 'E'.
     p_subrc = 4.
     EXIT.
   ENDIF.
   IF zcs_po_head-lifnr IS INITIAL.
     MESSAGE s000 WITH '字段:供应商  必输!'  DISPLAY LIKE 'E'.

     p_subrc = 4.
     EXIT.
   ENDIF.
   IF zcs_po_head-ekorg IS INITIAL.
     MESSAGE s000 WITH '字段:采购组织  必输!'  DISPLAY LIKE 'E'.
     p_subrc = 4.
     EXIT.
   ENDIF.
   IF zcs_po_head-ekgrp IS INITIAL.
     MESSAGE s000 WITH '字段:采购组  必输!'  DISPLAY LIKE 'E'.
     p_subrc = 4.
     EXIT.
   ENDIF.
   IF zcs_po_head-waers IS INITIAL.
     MESSAGE s000 WITH '字段:币别  必输!'  DISPLAY LIKE 'E'.
     p_subrc = 4.
     EXIT.
   ENDIF.
   IF zcs_po_head-mwskz IS INITIAL.
     MESSAGE s000 WITH '字段:税码  必输!'  DISPLAY LIKE 'E'.
     p_subrc = 4.
     EXIT.
   ENDIF.
